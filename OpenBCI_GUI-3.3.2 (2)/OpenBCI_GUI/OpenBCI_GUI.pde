...
String openBCI_portName = "COM0";  //starts as N/A but is selected from control panel to match your OpenBCI USB Dongle's serial/COM
...
OutputFile_BDF fileoutput_bdf_evt;
...
void initSystem() {
  ...
  switch (eegDataSource) {
    case DATASOURCE_CYTON:
      ...
      if (cyton.getInterface() == INTERFACE_SERIAL) {
        ...
        println("serial tranfer.....................................................");
    } else {
        println("wifi tranfer.....................................................");
  ...
  if (abandonInit) {
    ...
    ////open data file
    println("init start open new fileBDF................................");
    ...
    println("init end open new fileBDF................................");
    // wm = new WidgetManager(this);
    setupWidgetManager();
...
void stopButtonWasPressed() {
  ...
  if (isRunning) {
    ...
    stopRunning();
    println("eegDataSource = :"+eegDataSource);
    println("DATASOURCE_CYTON = :"+DATASOURCE_CYTON);
    ...
    wm.setUpdating(true);
        //open data file
    println("start open new fileBDF...........");
    if (eegDataSource == DATASOURCE_CYTON) openNewLogFile(fileName);  //open a new log file
    if (eegDataSource == DATASOURCE_GANGLION) openNewLogFile(fileName); // println("open ganglion output file");
    println("end open new fileBDF...........");
    startRunning();
...
void systemDraw() { //for drawing to the screen
  ...
  if (systemMode >= SYSTEMMODE_POSTINIT && !initSystemThreadLock) {
    ...
    //println("why resize GUI:"+(millis() - timeOfGUIreinitialize));
    //println("why resize GUI:"+reinitializeGUIdelay);
    ...
    if ((millis() - timeOfGUIreinitialize) > reinitializeGUIdelay) {
      ...
      try {
        ...
        println("161616161616:"+e.getMessage());
        reinitializeGUIdelay = reinitializeGUIdelay * 2;
