...
void clientEvent(Client someClient) {
  ...
  while(newChar != (char)-1) {
    ...
    //println("newChar----------------------------------------:"+newChar);
    //println("hub.tcpBufferPositon----------------------------------------:"+hub.tcpBufferPositon);
    if(p > 2) {
      ...
      if (posMatch.equals(TCP_STOP)) {
        ...
        //=======================msg:t,204,24,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,192,0,0,0,;
        //println("=======================msg:"+msg);
        //=======================len(msg):164
        //println("=======================len(msg):"+msg.length());
        //=======================hub.tcpBuffer:t
        //println("=======================hub.tcpBuffer:"+hub.tcpBuffer[0]);
        //=======================hub.tcpBuffer:
        //println("=======================hub.tcpBuffer:"+hub.tcpBuffer[(hub.tcpBuffer.length-1)]);
...
class Hub {
  ...
  public void initDataPackets(int _nEEGValuesPerPacket, int _nAuxValuesPerPacket) {
    ...
    nAuxValuesPerPacket = _nAuxValuesPerPacket;
	  println("rczs - eeg chn num: " + nEEGValuesPerPacket + "aux num: " + nAuxValuesPerPacket);
  ...
  UDP udp = null;
  public void sendRawDataforUdp(String msg){
  	//println("rczs - ");
    //println(msg);
    if(udp == null){
      udp = new UDP(this, 9123);
      return;
    }

    udp.send(msg, "127.0.0.1", 8123);
  }

  public void processData(String msg) {
    try {
	  //println("rczs - ");
      //println("processData....:"+msg);
      sendRawDataforUdp(msg);
      String[] list = split(msg, ',');
      int code = Integer.parseInt(list[1]);
      int stopByte = 0xC0;
      if ((eegDataSource == DATASOURCE_GANGLION || eegDataSource == DATASOURCE_CYTON) && systemMode == 10 && isRunning) {
        //RESP_SUCCESS_DATA_SAMPLE = 204 
        //=======================msg:t,204,195,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,
        //-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,192,0,0,0,;
        if (Integer.parseInt(list[1]) == RESP_SUCCESS_DATA_SAMPLE) {
          ...
          for (int i = 0; i < nEEGValuesPerPacket; i++) {
            //dataPacket.values[i]=-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,-8388608,192,0,0,0,;
            dataPacket.values[i] = Integer.parseInt(list[3 + i]);
          }
          if (newAccelData) {
            println("newAccelData..........................");
            newAccelData = false;
            for (int i = 0; i < NUM_ACCEL_DIMS; i++) {
          ...
          switch (outputDataSource) {
            ...
            case OUTPUT_SOURCE_BDF:
              ...
              println("start write data to bdf file.........................");
              fileoutput_bdf.writeRawData_dataPacket(dataPacketBuff[curDataPacketInd]);
              println("start write.........................:"+dataPacketBuff[curDataPacketInd]);
              println("start write.........................:"+curDataPacketInd);
              break;
