...
class Checkbox {
  ...
  Checkbox(String label, float x, float y, float w, float h, int index) {
    this.label = label;
    this.x = x;
    this.y = y;
    this.w = w;
  ...
  void display() {
    ...
    //  if(i==this.index){
    //    w_timeSeries.channelBars[this.index].draw();
    //  }
    //}
      
    fill(isChecked ? color(0, 255,0) : 150);  // 使用 RGB 值来设置颜色
...
class CheckboxWindow {
  ...
  CheckboxWindow(float x, float y, float w, float h, int numCheckboxes) {
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;

    cols = int(sqrt(numCheckboxes));
    rows = (numCheckboxes + cols - 1) / cols;  

    checkboxes = new Checkbox[numCheckboxes];
    
    float checkboxWidth = w / cols;
    float checkboxHeight = h / rows;
    
    for (int i = 0; i < numCheckboxes; i++) {
      int col = i % cols;
      int row = i / cols;
      float checkboxX = x + col * checkboxWidth;
      float checkboxY = y + row * checkboxHeight;
      checkboxes[i] = new Checkbox("" + (i + 1), checkboxX, checkboxY, checkboxWidth, checkboxHeight, i);
  ...
  void mouseClicked(float mx, float my) {
    for (Checkbox checkbox : checkboxes) {
      if (checkbox.contains(mx, my)) {
        checkbox.toggle();
        checkboxStates[checkbox.index] = checkbox.isChecked();
      }
    }
  }
}
...
//Button button;
boolean[] checkboxStates = new boolean[64];
boolean showCheckboxWindow = false;
...
class TopNav {
  ...
  Button filtSelectButton;
  Button filtFileButton;
  ...
  void initSecondaryNav(){
    ...
    filtSelectButton = new Button(15 + stopButton.but_dx + 140, 35, 70, 26, "Select\nChannel", fontInfo.buttonLabel_size);
    filtSelectButton.setFont(p5, 12);
    
    
    filtFileButton = new Button(19 + stopButton.but_dx + 210, 35, 70, 26, "Select\nTime", fontInfo.buttonLabel_size);
    filtFileButton.setFont(p5, 12);
  ...
  void updateSecondaryNavButtonsColor(){
    if(colorScheme == COLOR_SCHEME_DEFAULT){
      ...
      filtSelectButton.setColorNotPressed(color(255));
      filtFileButton.setColorNotPressed(color(255));

      filtBPButton.textColorNotActive = color(bgColor);
      filtNotchButton.textColorNotActive = color(bgColor);
      filtSelectButton.textColorNotActive = color(bgColor);
      filtFileButton.textColorNotActive = color(bgColor);
      ...
      filtSelectButton.setColorNotPressed(color(57,128,204));
      filtFileButton.setColorNotPressed(color(57,128,204));
      ...
      filtSelectButton.textColorNotActive = color(255);
      filtFileButton.textColorNotActive = color(255);
  ...
  void draw(){
    //println(nchan);
    checkboxWindow = new CheckboxWindow(160, 64, 300, 300, nchan);  
    pushStyle();
    ...
    if (showCheckboxWindow) {
        checkboxWindow.display();
      }
  }
  ...
  void mousePressed(){
    ...
    if(systemMode >= SYSTEMMODE_POSTINIT){
      ...
      if (filtSelectButton.isMouseHere()) {
        showCheckboxWindow = !showCheckboxWindow;
      }
      if (showCheckboxWindow) {
         checkboxWindow.mouseClicked(mouseX, mouseY);    
      }
      if(!filtSelectButton.isMouseHere()){
