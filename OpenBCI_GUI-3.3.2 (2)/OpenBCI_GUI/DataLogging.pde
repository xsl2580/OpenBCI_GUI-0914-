...
void openNewLogFileBDF(String _fileName) {
  ...
  //fileoutput_bdf_evt = new OutputFile_BDF(getSampleRateSafe(), nchan, _fileName);
...
public class OutputFile_BDF {
  ...
  OutputFile_BDF(float _fs_Hz, int _nbChan, String _fileName) {
    ...
    //fname_evt = getFileName(_fileName)[1];
    fs_Hz = (int)_fs_Hz;
    nbChan = _nbChan;
  ...
  public void writeRawData_dataPacket(DataPacket_ADS1299 data) {
    ...
    if (eegDataSource == DATASOURCE_CYTON) {
      println("DATASOURCE_CYTON.................");
      println("data.rawAuxValues:"+data.rawAuxValues);
      writeAuxDataValues(data.rawAuxValues);
    }
    samplesInDataRecord++;
    println("samplesInDataRecord:"+samplesInDataRecord);
    // writeValues(data.auxValues,scale_for_aux);
    if (samplesInDataRecord >= fs_Hz) {
      println("chanValBuf_buffer = .......................................");
    ...
    try {
      ...
      for (int i = 0; i < nbChan; i++) {
        for (int j = 0; j < fs_Hz; j++) {
          for (int k = 0; k < 3; k++) {
            //println("chanValBuf_buffer:"+chanValBuf_buffer[i][j][k]);
      ...
      if (eegDataSource == DATASOURCE_CYTON) {
        for (int i = 0; i < nbAux; i++) {
          for (int j = 0; j < fs_Hz; j++) {
            for (int k = 0; k < 3; k++) {
              //println("auxValBuf_buffer:"+auxValBuf_buffer[i][j][k]);
  ...
  private String getFileName(String s) {
    ...
    String output_data = "SavedData"+System.getProperty("file.separator")+"data-";
    //String output_evt = "SavedData"+System.getProperty("file.separator")+"evt-";
    output_data += s;
    //output_evt += s;
    output_data += ".bdf";
    //output_evt += ".bdf";
  ...
  //private String getFileName_evt(String s) {
  //  //String output = "SavedData"+System.getProperty("file.separator")+"OpenBCI-BDF-";
  //  String output = "SavedData"+System.getProperty("file.separator")+"evt-";
  //  output += s;
  //  output += ".bdf";
  //  return output;
  //}
