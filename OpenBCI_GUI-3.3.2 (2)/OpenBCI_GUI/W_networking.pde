       ...
       for(int i=0;i<nPointsPerUpdate;i++){
         ...
         for (int j = 0; j < numChan; j++){
           ...
           if (j != numChan-1) {
             ...
             outputter += ",";
             outputter += unixTimestamp;
             outputter += "]}\r\n";
           }
         }
         //outputter += ;
         try {
           println("outputter:"+outputter);
  ...
  void openNetwork(){
    ...
    if(this.protocol.equals("OSC")){
      ...
      info_data = new LSL.StreamInfo(
       ...
       //println("this.streamName:"+this.streamName);
       //println("this.streamType:"+this.streamType);
       //println("this.nChanLSL:"+this.nChanLSL);
       //println("getSampleRateSafe():"+this.streamType);
       outlet_data = new LSL.StreamOutlet(info_data);
      
       while (true) {
         float[] eegData = new float[this.nChanLSL]; // Replace with your data source
              //eegData[0] = 10.0;
              //eegData[1] = 10.0;
              //eegData[2] = 10.0;
              //eegData[3] = 10.0;
              //eegData[4] = 10.0;
          eegData = dataBuffY_filtY_uV[nchan-1];
          println("===============================:"+eegData[0]);
          outlet_data.push_sample(eegData);   // Send EEG data to the LSL stream
              // Add a delay or sleep to control the data sending rate
          try {
