...
//public void saveDataToDBF(String filePath, double data) {
//    try {

//        File dbfFile = new File(filePath);
//        Sheet sheet;
//        if (dbfFile.exists()) {
//            sheet = SpreadSheet.createFromFile(dbfFile).getSheet(0);
//        } else {
//            sheet = SpreadSheet.createEmpty(new File(filePath)).getSheet(0);
//        }

//        int lastRow = sheet.getRowCount();
//        sheet.setValueAt(data, 0, lastRow);


//        sheet.getSpreadSheet().saveAs(dbfFile);
//    } catch (IOException e) {
//        e.printStackTrace();
//    }
//}
  ...
  void update(){
    if(visible && updating){
      ...
      for(int i = 0; i < numChannelBars; i++){
       ...
       //saveDataToDBF("0901.dbf", channelBars[0].filt_uV_value);
  ...
  void updatePlotPoints(){
    ...
    if(dataBuffY_filtY_uV[channelNumber-1].length > nPoints){
      for (int i = dataBuffY_filtY_uV[channelNumber-1].length - nPoints; i < dataBuffY_filtY_uV[channelNumber-1].length; i++) {
        ...
        this.filt_uV_value = dataBuffY_filtY_uV[channelNumber-1][i];
        //this.filt_uV_value = 10.0;
